...
/**
 * Looks up an order by its idempotency key, scoped to the same customer
 * (authId when logged in, email for guest orders), and returns it in
 * CreateOrderResult shape. Null when none exists.
 */
async function _findOrderByIdempotencyKey(
  ...
  const customerScope = customerAuthId
    ? { "customer.customerAuthId": { equals: customerAuthId } }
    : { "customer.customerEmail": { equals: customerEmail } };
  const result = await payload.find({
    collection: "orders",
    where: {
      and: [{ idempotencyKey: { equals: idempotencyKey } }, customerScope],
    },
    limit: 1,
    depth: 0,
    overrideAccess: true,
